...
#alias dev_clean='LibriSpeech_dev_clean'
#alias test_clean='LibriSpeech_test_clean'
#alias clean100='LibriSpeech_clean100'
#alias clean360='LibriSpeech_clean360'
...
for n_src in $(yq r "${conf}" 'n_src'); do
    metadata_dir=metadata/Libri$n_src"Mix"
    echo python scripts/create_librimix_from_metadata.py --librispeech_dir $librispeech_dir \
    --wham_dir $wham_dir \
    --metadata_dir $metadata_dir \
    --librimix_outdir $librimix_outdir \
    --n_src $n_src \
    --freqs $(yq r "${conf}" 'freqs') \
    --modes $(yq r "${conf}" 'modes') \
    --types $(yq r "${conf}" 'types')
done
